	...
	for source in sources:
		for name in os.listdir(os.path.join("input", source)):
			with open(os.path.join("input", source, name), "r") as file:
				tokenizer.fit_on_texts([list(file)])
...
def input_generator():
	...
	while True:
		...
		for source, name in articles:
			with open(os.path.join("input", source, name), "r") as file:
				yield (numpy.array(tokenizer.texts_to_sequences([list(file)])),
...
model.fit_generator(input_generator(), tokenizer.document_count, callbacks=[
